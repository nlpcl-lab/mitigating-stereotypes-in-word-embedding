...
sensitive_pair, neutral_word_list = load_analogy_pair(SOURCE_DIR + 'minority_groups.txt')
...
def word2rep(_X_train, _y_train, model):
    X_train, y_train = [], []
    avg_fnl_weight = np.array([e[2] for e in _X_train]).mean()
    avg_hpw_weight = np.array([e[12] for e in _X_train]).mean()
    ...
    for X, y in zip(_X_train, _y_train):
        ...
        for token in X:
            if not re.search(r'[a-zA-Z\-?]+', token):
                ...
                #tmp_X = np.append(tmp_X, np.array([float(token)*avg_hpw_weight/avg_hpw_weight]))
...
if __name__ == "__main__":
    ...
    deb = DebiasModel(deepcopy(w2v.get_keyedvectors()), same_env=True)
